...
def get_latest_deployed_version(region, package):
    ...
    response = table.query(
        KeyConditionExpression=Key("deployed_region-package").eq(f"{region}.{package}"),
        Limit=1,
...
def check_latest_deploy(package, region, requirements_hash):
    ...
    (
        ...
        last_deployed_requirements_hash,
    ) = get_latest_deployed_version(region=region, package=package)
...
def main(event, context):
    ...
    s3.meta.client.download_file(
        os.environ["BUCKET_NAME"], package_artifact, f"/tmp/{package_artifact}"
    )
    ...
    for region in regions:
        ...
        if check_latest_deploy(
            ...
            try:
                ...
                logger.error(
                    "Unexpected error Writing to DB: {}".format(
